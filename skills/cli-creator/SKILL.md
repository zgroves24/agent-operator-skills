...
- Install name: a short binary name such as `ci-logs`, `admin-cli`, `alerts-cli`, or `build-logs`.

Prefer a new folder such as `~/code/clis/<tool-name>` or `~/tools/<tool-name>` when the user wants a personal tool and has not named a repo.
