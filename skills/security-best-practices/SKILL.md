...
While these references contain security best practices for languages and frameworks, projects may have cases where they need to bypass or override these practices. Pay attention to specific rules and instructions in the project's documentation and prompt files which may require you to override certain best practices. When overriding a best practice, you MAY report it to the user, but do not fight with them. If a security best practice needs to be bypassed or ignored for some project-specific reason, suggest adding documentation so future reviewers understand why the practice is not being followed.
...
While TLS is important for production deployments, most development work will have TLS disabled or provided by some out-of-scope TLS proxy. Be careful about reporting lack of TLS as a security issue. Also be careful around use of "secure" cookies. They should only be set if the application will actually run over TLS. If they are set on non-TLS applications, such as local development or testing, they can break the application. You can provide an environment flag to keep secure-cookie behavior off until a TLS production deployment. Avoid recommending HSTS by default. It is dangerous to use without full understanding of the lasting impacts, including outages and user lockout.
